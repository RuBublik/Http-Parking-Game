...
def run_tests():
    ...
    car = solution("POST", "/sessions", 201, {"plate": "12-345-67", "spotId": 106, "ev": True},
    ...
    session_id = car.get("id", MISSING)  # returned when the car parked; levels 7-10 use it

    level(7, "the EV charges 20 kWh")
    wrong("PATCH", f"/sessions/{session_id}", 400, {"chargedKwh": "20"})
    wrong("PATCH", "/sessions/1", 409, {"chargedKwh": 20})  # not an EV
    wrong("PATCH", f"/sessions/{session_id}", 200, {"chargedKwh": 10})  # a real charge, but not 20 kWh
    hint_is("PATCH", f"/sessions/{session_id}", body={"chargedKwh": 20})
    solution("PATCH", f"/sessions/{session_id}", 200, {"chargedKwh": 20}, chargedKwh=20, amount=60)
    verify("GET", f"/sessions/{session_id}", 200, chargedKwh=20, amount=60)

    level(8, "how much does the driver owe?")
    wrong("GET", "/sessions/6", 200)  # another car
    wrong("GET", "/sessions/abc", 404)
    hint_is("GET", f"/sessions/{session_id}")
    solution("GET", f"/sessions/{session_id}", 200, amount=60)
    solution("GET", "/spots/106/session", 200, amount=60)  # asking the spot works too

    level(9, "the driver pays what is due")
    wrong("DELETE", f"/sessions/{session_id}", 409)  # can't leave before paying
    wrong("POST", f"/sessions/{session_id}/payment", 402, {"amount": 59})  # not enough
    wrong("POST", f"/sessions/{session_id}/payment", 400, {"amount": "60"})
    hint_is("POST", f"/sessions/{session_id}/payment", body={"amount": 60})
    solution("POST", f"/sessions/{session_id}/payment", 200, {"amount": 60}, paid=True, change=0)
    verify("GET", f"/sessions/{session_id}", 200, paid=True)

    level(10, "the car leaves the lot")
    wrong("DELETE", "/sessions/3", 409)  # another car, not paid
    wrong("DELETE", "/spots/106", 405)
    hint_is("DELETE", f"/sessions/{session_id}")
    solution("DELETE", f"/sessions/{session_id}", 204)
    verify("GET", f"/sessions/{session_id}", 404)
    ...
    wrong("DELETE", f"/sessions/{session_id}", 404)  # that car already left
